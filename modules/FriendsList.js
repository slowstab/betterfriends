...
module.exports = async function () {
	...
	const _injectTabBar = async () => {
		...
		inject('bf-friendslist-tabbar', TabBar, 'render', (_, res) => {
	...
	const _injectFriendRow = async () => {
		...
		inject('bf-friendslist-row', FriendRow, 'render', (_, res) => {
			...
			res.props.children = (...args) => {
				...
				children.props.children.splice(1, 0, React.createElement('div', { className: 'bf-mutualGuilds bf-container', onClick: console.log }));
	...
	const _injectPeopleList = async () => {
		...
		const PeopleListSectionedNonLazy = await getModule(m => m.default?.displayName === 'PeopleListSectionedNonLazy');
		inject('bf-friendslist', PeopleListSectionedNonLazy, 'default', (_, res) => {
				...
				children.props.children[0].props.children = children.props.children[0].props.children.map(section => {
					...
					if (this.sortKey) {
						section = section.map(user => {
							...
							user.isFavorite = this.FAV_FRIENDS.some(f => f.id === user.key);
							return user;
						});
