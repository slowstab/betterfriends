...
module.exports = async function () {
  ...
  const ConnectedPrivateChannelsList = await getModule(
    (m) => m.default && m.default.displayName === "ConnectedPrivateChannelsList"
  );
  ...
  const ConnectedPrivateChannel = Flux.connectStores(
    ...
    ({ userId, currentSelectedChannel }) => {
      ...
      return {
        ...
        channel,
        selected,
        channelName: nickStore.getNickname(userId) ?? user.username,
  ...
  inject(
    ...
    (args, res) => {
      res.props.privateChannelIds = res.props.privateChannelIds.filter((c) => {
        ...
        return (
          channel.type !== 1 ||
          !this.FAV_FRIENDS.includes(channel.recipients[0])
        );
      });
